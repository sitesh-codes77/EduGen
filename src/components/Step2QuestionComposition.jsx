  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Total Questions */}
        <div style={{ backgroundColor: 'var(--card-2)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1rem 1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '0.6rem' }}>
            <Hash size={14} color="#0EA5E9" />
            <span style={{ color: 'var(--text-3)', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Total Number of Questions</span>
          </div>
          <input
            ...
            style={{ width: '100%', backgroundColor: 'var(--input-bg)', border: '1.5px solid var(--border)', borderRadius: '8px', padding: '10px 12px', fontSize: '1.4rem', fontWeight: 800, outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }}
            onFocus={(e) => { e.target.style.borderColor = '#0EA5E9'; e.target.style.boxShadow = '0 0 0 3px rgba(14,165,233,0.15)' }}
            onBlur={(e)  => { e.target.style.borderColor = 'var(--border)'; e.target.style.boxShadow = 'none' }}
          />
          <p style={{ color: !countOk && tq > 0 ? '#EF4444' : 'var(--text-3)', fontSize: '0.75rem', marginTop: '5px' }}>
        ...
        <div style={{ backgroundColor: 'var(--card-2)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1rem 1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '0.6rem' }}>
            ...
            <span style={{ color: 'var(--text-3)', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Total Marks for Paper</span>
          </div>
          <input
            id="totalMarksInput"
            className="text-slate-900 dark:text-slate-50"
            ...
            onBlur={(e)  => { e.target.style.borderColor = 'var(--border)'; e.target.style.boxShadow = 'none' }}
          />
          <p style={{ color: !marksOk && tm > 0 ? '#EF4444' : 'var(--text-3)', fontSize: '0.75rem', marginTop: '5px' }}>
      ...
      {/* ── Distribution table ── */}
      <div className="w-full overflow-x-auto" style={{ backgroundColor: 'var(--card-2)', border: '1px solid var(--border)', borderRadius: '12px' }}>
        <div style={{ minWidth: '500px' }}>
          {/* Table header */}
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 100px 100px auto', padding: '10px 16px', backgroundColor: 'var(--card)', borderBottom: '1px solid var(--border)', gap: '8px' }}>
            {['Question Type', 'Count', 'Mark', ''].map((h) => (
              <div key={h} style={{ color: 'var(--text-3)', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</div>
            ))}
          </div>

          {Q_TYPES.map((type, idx) => {
            const cfg     = data[type.key] || { count: 0, marks: type.defaultMarks }
            const subtotal = cfg.count * cfg.marks
            return (
              <div key={type.key}
                style={{ display: 'grid', gridTemplateColumns: '2fr 100px 100px auto', padding: '13px 16px', alignItems: 'center', gap: '8px', borderBottom: idx < Q_TYPES.length - 1 ? '1px solid var(--border)' : 'none', transition: 'background 0.15s' }}
                onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'var(--card)')}
                onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'transparent')}>
                <div>
                  <div style={{ color: 'var(--text-1)', fontWeight: 600, fontSize: '0.875rem' }}>{type.label}</div>
                  <div style={{ color: 'var(--text-3)', fontSize: '0.75rem', marginTop: '1px' }}>{type.description}</div>
                </div>
                {/* Count — inline editable */}
                <NumInput value={cfg.count} onChange={(v) => onTypeConfig(type.key, { count: Math.min(tq === '' ? 999 : tq - usedByOthers(type.key), v) })} min={0} width="72px" highlight={cfg.count > 0} />
                {/* Marks per Question */}
                <span style={{ color: 'var(--text-1)', fontWeight: 700, fontSize: '0.9rem' }}>{cfg.marks}</span>
                {/* Configure */}
                <motion.button onClick={() => setConfigOpen(type)}
                  whileHover={{ scale: 1.02 }}
                  style={{ display: 'flex', alignItems: 'center', gap: '5px', backgroundColor: 'var(--primary-dim)', border: '1px solid rgba(14,165,233,0.25)', borderRadius: '7px', padding: '6px 11px', color: '#0EA5E9', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'background-color 0.2s, border-color 0.2s' }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(14,165,233,0.2)'; e.currentTarget.style.borderColor = '#0EA5E9' }}
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'var(--primary-dim)'; e.currentTarget.style.borderColor = 'rgba(14,165,233,0.25)' }}>
